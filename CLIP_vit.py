...
# import ViT - note that this also import all component classes for the transformer (from utils_transformer)
from vit import *
...
class CLIP(nn.Module):
    def __init__(self, text_encoder, image_encoder, d_model, eos_token):
        ...
        # add projection layers - this is needed since the d_model of text_encoder can be different from d_model of image_encoder (the embedding spaces can be disjoint)
    ...
    def encode(self, imgs, cap_tokens, cap_pad_mask, cap_causal_mask):
        ...
        encoded_img_cls = encoded_img[:, 0] # encoded_img.shape: [batch_size, d_model]
        # project encoded_img to common embedding space
        clip_img_emb = self.image_proj(encoded_img_cls)
        ...
        # project encoded_txt to common embedding space
        clip_txt_emb = self.text_proj(encoded_txt_eos) # clip_txt_emb.shape: [batch_size, d_model]
    ...
    def forward(self, imgs, cap_tokens, cap_pad_mask, cap_causal_mask):
        ...
        clip_img_emb, clip_txt_emb, encoded_txt = self.encode(imgs, cap_tokens, cap_pad_mask, cap_causal_mask)
